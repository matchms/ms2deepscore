...
def test_load_safe_checkpoint(safe_checkpoint):
    """Test loading a safe checkpoint."""
    model = load_model(safe_checkpoint)
...
def test_load_legacy_model(legacy_checkpoint):
    """Test loading a legacy pickled model with allow_legacy=True."""
    model = load_model(legacy_checkpoint, allow_legacy=True)
    assert isinstance(model, SiameseSpectralModel)
    assert model.model_settings.base_dims == (100, 100)
    assert model.model_settings.embedding_dim == 10
...
def test_load_legacy_and_convert(legacy_checkpoint, tmp_path):
    """Test legacy model conversion into a safe format."""
    convert_path = tmp_path / "converted_model.pt"
    _ = load_model(legacy_checkpoint, allow_legacy=True, convert_legacy_to=convert_path)
