...
def test_DataGeneratorAllSpectrums_additional_inputs():
    ...
    test_cases = [(StandardScaler("precursor_mz", mean=0, std=1000), ),  # We are using two test cases, one with only one
                  (StandardScaler("precursor_mz", mean=0, std=1000), StandardScaler("precursor_mz", mean=0, std=100))]  # Normally you would of course have two different ones (but metadata did not allow it)
