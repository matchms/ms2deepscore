...
def create_plot(predictions, scores_ref, plots_folder, file_name):
    create_histograms_plot(scores_ref, predictions, n_bins=10, hist_resolution=100,
    ...
    filename = os.path.join(plots_folder,
                            f"{file_name}_plot.svg")
    plt.savefig(filename)


def create_reverse_plot(predictions, scores_ref, plots_folder, file_name):
    create_histograms_plot(predictions, scores_ref, n_bins=10, hist_resolution=100,
                           ref_score_name="MS2DeepScore (predicted Tanimoto similarity)", compare_score_name="Tanimoto similarity")
    ...
    filename = os.path.join(plots_folder,
                            f"{file_name}_plot_reversed.svg")
    plt.savefig(filename)


def benchmark_wrapper(val_spectra_1, val_spectra_2,
    ...
    predictions_file_name = os.path.join(benchmarking_results_folder,
                                         f"{file_name}_predictions.pickle")
    if os.path.exists(predictions_file_name):
        ...
        predictions = ms2ds_model.matrix(val_spectra_1, val_spectra_2,
                                         is_symmetric=is_symmetric)
        save_pickled_file(predictions, predictions_file_name)

    # Calculate true values
    true_values_file_name = os.path.join(benchmarking_results_folder, f"{file_name}_true_values.pickle")
...
def create_all_plots(model_folder_name):
    ...
    positive_validation_spectra = load_pickled_file(os.path.join(
        data_dir, "training_and_validation_split", "positive_validation_spectra.pickle"))
