...
def train_ms2deepscore_wrapper(data_directory,
    ...
    training_data_storing = StoreTrainingData(data_directory, spectra_file_name, validation_split_fraction)

    # Split training in pos and neg and create val and training split and select for the right ionisation mode.
    training_spectra, validation_spectra, _ = load_train_val_data(training_data_storing,
                                                                  settings.ionisation_mode)
    # Train model
    train_ms2ds_model(training_spectra, validation_spectra,
                      os.path.join(training_data_storing.trained_models_folder, settings.model_directory_name),
                      settings)

    create_all_plots(training_data_storing,
