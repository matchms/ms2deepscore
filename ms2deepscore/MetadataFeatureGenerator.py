...
class MetadataVectorizer:
    ...
    @property
    def size(self):
Here is the code with more extensive docstrings explaining the usage of additional_metadata and other details:
...
class MetadataVectorizer:
    ...
    def transform(self, spectra: List[SpectrumType], progress_bar=False):
        """Transforms the input spectra into metadata vectors as needed for MS2DeepScore.
        ...
        -------
        torch.Tensor
            A tensor containing the metadata vectors for the input spectra.
        """
        metadata_vectors = torch.zeros((len(spectra), self.size))
        for i, spec in tqdm(enumerate(spectra), desc="Create metadata vectors", disable=(not progress_bar)):
            metadata_vectors[i, :] = torch.tensor(
